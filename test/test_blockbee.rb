...
class TestBlockBee < Minitest::Test
  ...
  def test_can_generate_address
    blockbee_helper = BlockBee::API.new(COIN, OWN_ADDRESS, CALLBACK_URL, PARAMETERS, BB_PARAMETERS, API_KEY)
  ...
  def test_can_get_qrcode
    ...
    qrcode = blockbee_helper.get_qrcode(10, 300)
    # p qrcode
    refute_nil qrcode
  end
  ...
  def test_can_get_info
    info = BlockBee::API.get_info('btc', API_KEY, 1)
    # p info
    refute_nil info
  end
  ...
  def test_can_create_payouts
    payouts = BlockBee::API.create_payout('polygon_matic', {
  ...
  def test_can_list_payouts
    payouts_list = BlockBee::API.list_payouts('polygon_matic', status:'done', api_key: API_KEY)
    # p payouts_list
  ...
  def test_can_process_payout_by_id
    process = BlockBee::API.process_payout(API_KEY, 2593)
    # p process
    refute_nil process
  end

  def test_can_check_payout_status
    status = BlockBee::API.check_payout_status(API_KEY, 2593)
    p status
    refute_nil status
  end
